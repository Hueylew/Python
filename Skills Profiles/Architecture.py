...
for f in glob.glob('/Users/adamlewis/Documents/Work/Business Consultancy/Skills and Development/Combined Skills Profiles 2021/*.xlsx'):
	...
	PersonScore = sh["K22"].value
	PersonTarget = sh["K23"].value
	PersonVariance = sh["K24"].value
	...
	ws = wb["Architecture"]
	df_BA = load_workbook_range('A1:D77', ws)
	...
	#merge all the dataframes together into one large dataframe
	if index == 1:
		all_data = df_BA
	else:
...
all_data.to_excel(writer, "Architecture", index = False)
